...
export async function GET(request: Request, routeContext: RouteContext) {
  ...
  if (error) {
    console.error("Error loading employee blocked times:", error);
    return NextResponse.json(
      { error: "No se pudieron cargar las vacaciones del empleado." },
      { status: 500 }
    );
  }
...
export async function POST(request: Request, routeContext: RouteContext) {
  ...
  if (error) {
    console.error("Error creating employee blocked time:", error);
    return NextResponse.json(
      { error: "No se pudo crear el bloqueo del empleado." },
      { status: 500 }
    );
  }
